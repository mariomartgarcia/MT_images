...
#python MT_complete.py -epo 1 -bs 500 -pat 5 -iter 1
...
test_nir_ex = np.repeat(test_nir, 3, axis=-1)  
test_pri = np.concatenate([test_rgb, test_nir_ex], axis=1)
...
for k in ran:
    ...
    train_nir_ex = np.repeat(train_nir, 3, axis=-1)  
    train_pri = np.concatenate([train_rgb, train_nir_ex], axis=1)

    val_nir_ex = np.repeat(val_nir, 3, axis=-1)
    val_pri = np.concatenate([val_rgb, val_nir_ex], axis=1)
    ...
    train_concat = ut.concatenate_images(train_rgb, pre_train)
    val_concat = ut.concatenate_images(val_rgb, pre_val)
    test_concat = ut.concatenate_images(test_rgb, pre_test)
    ...
    #Save the results
    off = {'err_up':np.round(np.mean(err_up), 4),
...
dff.to_csv('Eurosat_' + str(epo) + '_' + str(bs) + '_' + str(pat)+ '_' + str(n_iter)+ '.csv')
